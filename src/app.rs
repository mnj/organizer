//! Classification GUI: maximized GTK4 split-view (Preview + Action Bar).
//!
//! Public seam: [`APPLICATION_ID`] and [`run`]. Callers pick CLI vs GUI in
//! `main`; this module owns the window, shortcuts, and Classification flow.
...
use crate::mover::{classify_file, mover_error_message};
use crate::preview::{ensure_sandbox_bwrap, is_glycin_supported, load_texture};
use crate::queue::build_snapshot;
use crate::settings::{open_settings, SettingsContext};
use crate::store::Store;
use crate::undo::{
    classification_target, push_undo_capped, undo_classification, UndoEntry,
};
use crate::video;
...
/// GTK/D-Bus application id — reverse-DNS of the GitHub repo `mnj/organizer`.
pub const APPLICATION_ID: &str = "io.github.mnj.Organizer";

/// Start the Classification GUI. `source_folder` is the CLI argument, or
/// `None` to show the folder picker.
pub fn run(source_folder: Option<PathBuf>) -> glib::ExitCode {
...
fn preload_next_preview(
    ...
    if cur + 1 < snapshot.len() {
        ...
        if crate::preview::is_glycin_supported(&nxt)
            && !cache.borrow().contains_key(&nxt)
            && nxt.exists()
        {
            let cache_clone = cache.clone();
            glib::MainContext::default().spawn_local(async move {
                if let Ok(t) = crate::preview::load_texture(&nxt, None).await {
...
fn build_shell(app: &Application, snapshot: Vec<PathBuf>, source_folder: PathBuf) {
    ...
    let show_toast = {
        ...
        move |msg: String| {
            ...
            t.set_visible(true);
            *gen.borrow_mut() += 1;
            let this_gen = *gen.borrow();
            let tt = t.clone();
            let gen_c = gen.clone();
            glib::timeout_add_seconds_local(3, move || {
                if *gen_c.borrow() == this_gen {
    ...
    let update_ui = {
        ...
        move || {
            ...
            let effective_path = classification_target(
                &snap_c,
                i,
                override_c.borrow().as_deref(),
            )
            .unwrap_or_else(|| snap_c[i].clone());
    ...
    let trigger_move: Rc<dyn Fn(Category)> = {
        ...
        Rc::new(move |category: Category| {
            ...
            let current = match classification_target(
                &snap_c,
                i,
                override_c.borrow().as_deref(),
            ) {
                ...
                glib::idle_add_local(move || {
                    match rx_cell_clone.borrow_mut().try_recv() {
                        ...
                        Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                            tracing::warn!("hash thread ended without a result for {}", file_name_err);
                            show_toast_err(format!("Failed to hash {}: hashing interrupted", file_name_err));
                            set_busy_state(&busy_c2, &spinner_c2, &categories_row_c2, &btn_prev_c2, &btn_next_c2, false);
                            update_ui_c2();
                            glib::ControlFlow::Break
                        }
...
mod tests {
    ...
    #[test]
    fn application_id_is_github_reverse_dns_not_example() {
        assert_eq!(APPLICATION_ID, "io.github.mnj.Organizer");
        assert!(!APPLICATION_ID.contains("example"));
    }

    #[test]
    fn hash_service_prehash_sync_caches_current_and_next() {
        let dir = tempfile::TempDir::new().unwrap();
        let a = dir.path().join("a.jpg");
        let b = dir.path().join("b.jpg");
        std::fs::write(&a, b"aaa").unwrap();
        std::fs::write(&b, b"bbb").unwrap();
        let snapshot = Rc::new(vec![a.clone(), b.clone()]);
        let idx = Rc::new(RefCell::new(0usize));
        let cache = Arc::new(Mutex::new(HashMap::new()));
        let hs = HashService::new(cache.clone(), snapshot, idx);
        hs.prehash_sync();
        let map = cache.lock().unwrap();
        assert_eq!(map.len(), 2);
        assert_eq!(map.get(&a).unwrap(), &compute_sha256(&a).unwrap());
        assert_eq!(map.get(&b).unwrap(), &compute_sha256(&b).unwrap());
    }
}
