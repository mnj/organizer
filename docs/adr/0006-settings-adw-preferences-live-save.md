...
Each Action row has `Display name` [Entry], `Folder name` [Entry, helper “sibling ../<folder>/, slug a-z0-9_-”], and `Shortcut` [DropDown 1–9 showing “1 or Ctrl+1”]; row controls are `Add Action` / `Remove` (×) / `Up`/`Down` reorder. Max 9 rows, min 1, no color/icon v1. The `Folder name` field live-sanitizes and previews the slug.
