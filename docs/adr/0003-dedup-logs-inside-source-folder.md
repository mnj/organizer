# Dedup logs inside Source Folder, duplicate subfolder, pre-hash next
...
Duplicate check is the **union** of all `<folder>.txt` logs (loaded into a `HashSet` at launch, incrementally updated). If the current hash exists in any log, we **auto-move to the `duplicate/` subfolder** (fixed name, lazy-created, inside the Source Folder) with toast “Duplicate — already in ‘keep’ — moved to duplicate/”, filename collision → suffix `_1`, `_2`, and do not append to a `duplicate.txt`. No confirmation dialog.
